    ...
    <div class="operateTableBox">
      ...
      @selection-change="handleSelectionChange">
        ...
        <el-table-column
          ...
          width="140">
          <template slot-scope="scope">
            {{scope.row.is_featured | getBoolean}}
            <el-switch
        ...
        <el-table-column
          ...
          width="100">
         <template slot-scope="scope">
            {{scope.row.is_sold | getBoolean}}
            <!-- <el-switch
              v-model="scope.row.is_sold"
              active-color="#409EFF"
              inactive-color="#c7c7c7"
              @change="soldChange($event,scope.row.id)">
            </el-switch> -->
          </template>
        </el-table-column>
        <el-table-column
...
export default {
  ...
  methods: {
      ...
      this.$axios.post(url,params).then((res)=>{
        ...
        if(result.status_code == ERR_OK){
          ...
          for(var i=0;i<that.tableData.length;i++){
            ...
            if(that.tableData[i].is_sold==1){
              that.tableData[i].is_sold = true
            }
          }
    ...
    edit(row) {
      var carId = row.id;
      this.$cookie.set("_carId",carId)
      this.$router.push('/commodityEdit?from=edit')
    },
